...
class create_motif_crispr(object):


    def create_fasta_file(self):

        fo =  open("/scratch/0/bharadwk/CRISPR/CRISPR/master_dataset/masterfile.txt", 'r')
        result = []
        tmp_item = ""
        start = 0
        end = 0
        ...
        for pos, line in enumerate(result):
			...
			start = int(start) - len(crispr_str)
			end = int(end) - len(crispr_str)
			start = start - 100
			end = end + 100
			result3 = "".join(result2)
			result3 = result3[start:end]

			path2 = "/scratch/0/bharadwk/CRISPR/CRISPR/master_dataset/master_fasta_file/%s__%s.fa" %(tmp_name, count)
			fo2 = open(path2, "a")
    ...
    def get_repeat_spacer_file(self):
            ...
            path3 = "/scratch/0/bharadwk/CRISPR/CRISPR/master_dataset/master_fasta_file/*.fa"
            dest_path1 = "/scratch/0/bharadwk/CRISPR/CRISPR/master_dataset/master_repeat/master_repeat_array.txt"
            dest_path2 = "/scratch/0/bharadwk/CRISPR/CRISPR/master_dataset/master_spacer/master_spacer_array.txt"
            ...
            for name in files:
				...
				if num_lines > 1:
					cmd = "java -cp /scratch/0/bharadwk/CRISPR/bin/CRT1.2-CLI.jar crt %s /scratch/0/bharadwk/CRISPR/CRISPR/master_dataset/master_crispr/CRISPRs_Repeats_Spacers_%s.out" %(name,tmp_name)
					sp.call(cmd, shell=True)
					path4 = "/scratch/0/bharadwk/CRISPR/CRISPR/master_dataset/master_crispr/CRISPRs_Repeats_Spacers_%s.out" %(tmp_name)
					tmp1 = path4.split('.')[0]
